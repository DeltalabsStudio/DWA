...
public class ColorPickerPalette extends TableLayout {
    ...
    public void init(int size, int numColumns, ColorPickerSwatch.OnColorSelectedListener onColorSelectedListener) {
        ...
        if (size == 1) {
            mSwatchLength = resources.getDimensionPixelSize(Tools.getID("color_swatch_large", "dimen"));
            mMarginSize = resources.getDimensionPixelSize(Tools.getID("color_swatch_margins_large", "dimen"));
        } else {
            mSwatchLength = resources.getDimensionPixelSize(Tools.getID("color_swatch_small", "dimen"));
            mMarginSize = resources.getDimensionPixelSize(Tools.getID("color_swatch_margins_small", "dimen"));
        }
        mOnColorSelectedListener = onColorSelectedListener;
        mDescription = Tools.getString("color_swatch_description");
        mDescriptionSelected = Tools.getString("color_swatch_description_selected");
    }
}
