...
import id.delta.whatsapp.value.Avatar;
...
public class RoundedImageView extends ImageView {
    ...
    public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mLeftTopCornerRadius = Avatar.leftTop();
        mRightTopCornerRadius = Avatar.rightTop();
        mLeftBottomCornerRadius = Avatar.leftBottom();
        mRightBottomCornerRadius = Avatar.rightBottom();
        ...
        mBorderWidth = Avatar.lebarBorder();
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = ColorStateList.valueOf(Avatar.warnaBorder());
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = Avatar.setCircleView();
    ...
    private void updateAttrs(Drawable drawable, ScaleType scaleType) {
        ...
        if (drawable instanceof RoundedCornerDrawable) {
            ...
            ((RoundedCornerDrawable) drawable).setCornerRadii(mRadii);
            ((RoundedCornerDrawable) drawable).setBorderWidth(mBorderWidth);
            ((RoundedCornerDrawable) drawable).setBorderColor(mBorderColor);
            ((RoundedCornerDrawable) drawable).setOval(isOval);
